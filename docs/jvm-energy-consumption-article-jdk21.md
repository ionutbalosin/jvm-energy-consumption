...
Even though `wrk`'s reported latency histogram suffers from [coordinated omission](https://www.scylladb.com/2021/04/22/on-coordinated-omission), latency was not the focus of my measurements. Instead, I relied on the load generated by `wrk`, including the maximum throughput, as key factors to assess the energy consumption of a highly loaded system.
